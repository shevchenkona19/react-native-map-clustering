...
import { Dimensions, LayoutAnimation, Platform, useCallback, useMemo } from "react-native";
...
const ClusteredMapView = forwardRef(
  (
    ...
    const _onRegionChangeComplete = useCallback(
      ...
      [clusterChildren, updateSpiderfier, updateMarkers, onMarkersChange, onRegionChangeComplete, updateRegion]
    );
    ...
    const _onClusterPress = useCallback(
      ...
      [updateClusterChildren, onClusterPress, children, mapRef.current]
    );
    ...
    }, [markers, propsChildren, restProps, clusterColor, clusterTextColor, clusterFontFamily, tracksViewChanges]);

    const _spiderMarkers = useMemo(() => {
    ...
    }, [spiderMarkers, propsChildren]);

    const _moreSpiderMarkers = useMemo(() => {
